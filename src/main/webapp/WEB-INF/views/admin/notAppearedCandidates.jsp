...
<title>Analysis Report For Not Appeared Candidates</title>
      ...
      selected = 'selected' </c:if>>${pv.testTakerName}</option>
              ...
              <div class="card-body">
                <table id="example" class="table table-bordered table-striped">
                  ...
                  <tbody>
						<c:choose>
							<c:when test="${empty allNotAppearedCandidates}">
								<tr>
									<td colspan="9">No data available</td>
								</tr>
							</c:when>
							<c:otherwise>
								<c:forEach items="${allNotAppearedCandidates}" var="cand" varStatus="count">
									<tr>
										<td>${count.count}</td>
						...
						"buttons" : [ {
							...
							title : function() {
								...
								if (collegeName != '--select--'
										&& batchName != '--select--') {
									return 'Analysis Report of Not Appeared Candidate- '
											+ collegeName + ' - '
											+ batchName;
									...
									return 'Analysis Report - of Not Appeared Candidates '
								}
							}
						} ]
					});
